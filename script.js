...
codeToggleBtn.addEventListener("click", () => {
  const shouldShow = sourceCodePanel.hidden;

  sourceCodePanel.hidden = !shouldShow;
  codeToggleBtn.textContent = shouldShow ? "Hide Kruskal C Code" : "Show Kruskal C Code";

  if (shouldShow) {
    sourceCodePanel.scrollIntoView({ behavior: "smooth", block: "start" });
  }
});
